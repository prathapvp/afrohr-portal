        ...
        >
            {
                jobCategory.map((category, index) => <Carousel.Slide key={index}>
                    <div data-aos="zoom-out" className="flex flex-col items-center w-64 sm-mx:w-56 xs-mx:w-48 gap-2 border border-bright-sun-400 p-5 rounded-xl hover:cursor-pointer hover:shadow-[0_0_5px_2px_black] my-5 transition duration-300 ease-in-out !shadow-bright-sun-300 glass-card elevated-card">
                        <div className="p-2 bg-bright-sun-300 rounded-full icon-container">
                            <img className="h-8 w-8 sm-mx:h-6 sm-mx:w-6 xs-mx:h-4 xs-mx:w-4 img-polished" src={`/Category/${category.name}.png`} alt={category.name} />
                        </div>
